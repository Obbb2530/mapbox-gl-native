...
struct SectionOptions {
    ...
    SectionOptions(std::string imageID_) : scale(1.0), imageID(std::move(imageID_)) {}
...
private:
    ...
    // Max number of images within a text is 6401 U+E000–U+F8FF
    // that covers Basic Multilingual Plane Unicode Private Use Area (PUA).
    optional<char16_t> imageSectionID;
};
