...
public:
    ...
    void getTile(const CanonicalTileID& id, std::function<void(TileFeatures)> fn) final {
        assert(fn);
        // It's safe to pass `this` as scheduler will die earlier.
        scheduler.scheduleAndReplyValue(
            [id, this]() -> TileFeatures { return impl.getTile(id.z, id.x, id.y).features; }, std::move(fn));
    }
